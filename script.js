...
function updateSummary() {
    ...
    appData.buddies.forEach(buddy => {
        ...
        appData.expenses.forEach(item => {
            if (item.type === 'divider') {
                // If a previous day was open, close its container
                ...
                // Change: Conditional styling.
                // If it's 'Day 1', we REMOVE the top border and top margin/padding.
                // For all other days, we keep the separator line.
                const borderClasses = activeDayLabel === 'Day 1' 
                    ? 'mb-1' // Minimal spacing for the first item
                    : 'pt-2 border-t border-slate-200 mt-2'; 

                details[buddy].push(`
                    <div class="text-xs font-bold text-sky-700 cursor-pointer flex items-center justify-between day-toggle-btn ${borderClasses}" data-day="${activeDayLabel}">
                        <span>${activeDayLabel}${subtotalStr}</span>
                        <i class="fas ${icon} ml-2 text-[10px]"></i>
                    </div>
                ...
                if (exp.buddies.includes(buddy)) {
                    ...
                    // UPDATED LINE BELOW: Added <span class="font-bold text-slate-700"> around the amount
                    details[buddy].push(`
        ...
        // Close the last open day container
        if (activeDayLabel !== null) details[buddy].push('</div>');
    });

    if (appData.buddies.length === 0) {
        summaryTableBody.innerHTML = `<tr><td colspan="3" class="py-4 px-2 text-center text-slate-500">No buddies added yet.</td></tr>`;
        return;
    }

    // Step 3: Populate Table Rows
    // Kept 'align-middle' as requested previously for vertical center alignment
    appData.buddies.forEach(buddy => {
        const row = summaryTableBody.insertRow();
        row.innerHTML = `
            <td class="py-3 px-2 md:px-4 text-slate-700 font-medium align-middle whitespace-nowrap">${buddy}</td>
            <td class="py-3 px-2 md:px-4 text-slate-700 align-middle whitespace-nowrap">${currencySymbol}${formatCurrency(totals[buddy])}</td>
            <td class="py-3 px-2 md:px-4 text-slate-700 space-y-1 align-middle">${details[buddy].join('') || '<span class="text-slate-400">No records</span>'}</td>
        `;
    });
}

    ...
    if (selectedValue === 'CUSTOM') {
        ...
        supportedCurrencies.forEach(code => {
            if (code !== currentPrimary) {
                ...
                // CHECK: Is there a saved custom rate for this currency?
                if (appData.customRates && appData.customRates[code]) {
                    option.textContent = `${code} *`; // Add visual indicator
                    option.classList.add('font-bold', 'text-sky-700'); // Optional styling for supported browsers
                    
                    // Capture the first saved currency found to auto-select it later
        ...
        // 3. Auto-Select Logic
        // If we found a saved currency, select it. Otherwise, default to the first in the list.
        if (firstSavedCurrency) {
            customRateSource.value = firstSavedCurrency;
        }

        // 4. Set Target Input
        customRateTarget.value = currentPrimary;

...
tripPrimaryCurrencySelect.addEventListener('change', () => {
    ...
    // (It will update properly after the async operation finishes)
    tripPrimaryCurrencySelect.value = oldCurrency;

    const message = `Change trip currency to ${newCurrency}? \n\n⚠️ This will recalculate all existing expenses based on current exchange rates.`;
    
    showConfirmation(
        message,
        async () => { // Made this callback ASYNC
            try {
                showLoading("Fetching rates & recalculating...");

                // 1. Update the local state first so fetchExchangeRates uses the NEW currency
                appData.primaryCurrency = newCurrency;

                // 2. Fetch new rates based on the new Primary Currency
                await fetchExchangeRates();

                // 3. Iterate through expenses and recalculate values
                // We use map to create a new array with updated values
                appData.expenses = appData.expenses.map(item => {
                    if (item.type === 'expense') {
                        // Get the rate for the EXPENSE'S original currency relative to the NEW primary currency
                        // Example: Expense was 100 USD. New Primary is HKD.
                        // exchangeRates['USD'] will be ~7.8.
                        // Calculation: 100 / (1/7.8) ... Wait, logic check:
                        
                        // Your script logic (line 1042) is: amountInPrimaryCurrency = amount / (exchangeRates[currency] || 1);
                        // The API returns rates where Base = Primary.
                        // So if Base = HKD, then Rate for USD is ~0.128.
                        // 100 USD / 0.128 = 780 HKD. Correct.
                        
                        const rate = exchangeRates[item.currency] || 1;
                        const newAmountInPrimary = item.amount / rate;
                    ...
                    return item; // Return dividers unchanged
                });

                // 4. Save the new Currency AND the recalculated Expenses to Firebase
                saveDataToFirebase();

                showError(`Converted trip to ${newCurrency}. Expenses updated.`, 3000, 'success');

            } catch (error) {
                console.error("Currency switch error:", error);
                showError("Failed to recalculate expenses. Reverting.");

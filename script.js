...
const authLoadingIndicator = document.getElementById('auth-loading-indicator'); // New: Loading indicator
...
loginForm.addEventListener('submit', e => {
    ...
    signInWithEmailAndPassword(auth, email, password)
        .catch(error => {
            ...
            authLoadingIndicator.classList.add('hidden'); // Hide loading indicator on error
        });
});
    ...
    trips.forEach(trip => {
        ...
        item.innerHTML = `
            ...
            <div class="flex items-center gap-3 shrink-0">
                <button class="copy-trip-link-btn text-slate-500 hover:text-sky-500 transition-colors" data-trip-id="${trip.id}" data-trip-name="${trip.name}" aria-label="Copy link for ${trip.name}">
                    <i class="fa-regular fa-copy text-base"></i>
                </button>
                <button class="share-whatsapp-btn text-green-500 hover:text-green-700 transition-colors" data-trip-id="${trip.id}" data-trip-name="${trip.name}" aria-label="Share on WhatsApp">
...
function attachTripSwitcherListeners() {
    ...
    document.querySelectorAll('.share-whatsapp-btn').forEach(btn => {
        btn.addEventListener('click', (event) => {
            event.stopPropagation();
            const tripIdToShare = event.currentTarget.dataset.tripId;
            const tripName = event.currentTarget.dataset.tripName;
            if (tripIdToShare) {

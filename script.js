    ...
    trips.forEach(trip => {
        ...
        item.innerHTML = `
            ...
            <div class="flex items-center gap-3 shrink-0">
                <button class="copy-trip-link-btn text-slate-500 hover:text-sky-500 transition-colors" data-trip-id="${trip.id}" data-trip-name="${trip.name}" aria-label="Share link for ${trip.name}">
...
tripPrimaryCurrencySelect.addEventListener('change', async () => {
    ...
    // Immediately revert the dropdown visually (wait for confirmation)
    tripPrimaryCurrencySelect.value = oldCurrency;

    try {
        showLoading("Checking exchange rates...");

        // 1. Fetch "Preview" Rates for the NEW currency
        let newRates = {};
        try {
            const response = await fetch(`https://v6.exchangerate-api.com/v6/${API_KEY}/latest/${newCurrency}`);
            const data = await response.json();
            if (data.result === 'success' && data.conversion_rates) {
            ...
            const primaryInUsd = fallbackRates[newCurrency];
            for (const key in fallbackRates) {
        ...
        // 2. Calculate the Live Rate for display
        // Logic: newRates is based on New Currency. 
        // newRates[oldCurrency] = Amount of Old Currency per 1 New Currency.
        // We want to display: 1 Old = X New.
        const rateOldInNewBase = newRates[oldCurrency] || 1; 
        const liveRate = 1 / rateOldInNewBase; 

        // 3. Construct the HTML Message (Clean, no historical stats)
        const message = `
            Switching from <strong>${oldCurrency}</strong> to <strong>${newCurrency}</strong>.
            <div class="bg-slate-50 p-3 rounded-md mt-3 mb-3 text-sm border border-slate-200 text-center">
                ...
                <span class="font-mono font-bold text-slate-700">1 ${oldCurrency} = ${liveRate.toFixed(4)} ${newCurrency}</span>
            </div>
            <p class="text-xs text-slate-500">⚠️ All existing expenses will be recalculated using this rate.</p>
        `;

        showConfirmation(
            "", // Title left empty, logic handles HTML injection below
            async () => {
                // ON CONFIRM
                try {
                    ...
                    // A. Update State
                    appData.primaryCurrency = newCurrency;
                    exchangeRates = newRates; // Use the rates we just fetched

                    // B. Recalculate Expenses
                    appData.expenses = appData.expenses.map(item => {
                        if (item.type === 'expense') {
                            // Convert Original Currency -> New Primary using LIVE rates
                            ...
                            return {
                                ...item,
                                amountInPrimaryCurrency: newAmountInPrimary,
                                perBuddy: newPerBuddy
                            };
                        }
                        return item;
                    });
                    ...
                    // D. Update UI Dropdown
                    tripPrimaryCurrencySelect.value = newCurrency;
                    
                    showError(`Converted to ${newCurrency}. Expenses updated.`, 3000, 'success');

                } catch (error) {
                    console.error("Conversion error:", error);
                    showError("Error converting expenses.");
                    appData.primaryCurrency = oldCurrency;
                    tripPrimaryCurrencySelect.value = oldCurrency;
                    await fetchExchangeRates(); // Restore old rates
                } finally {
                    hideLoading();
                }
            },
        ...
        // Inject the HTML message into the modal
        setTimeout(() => {

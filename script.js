                ...
                if (exp.buddies.includes(buddy)) {
                    ...
                    details[buddy].push(`
                        <div class="text-xs text-slate-600">
                            <span class="font-medium">${exp.mealType}</span> at ${exp.restaurant}: ${currencySymbol}${formatCurrency(exp.perBuddy)}
                        </div>
                    `);
                }
            }
...
function updateAdjustmentsDisplay() {
    ...
    appData.adjustments.forEach((adj, index) => {
        ...
        // Changed: Removed 'flex' from wrapper to allow vertical stacking of content vs footer
        adjDiv.className = 'relative bg-white p-3.5 rounded-lg shadow-sm border border-slate-200 text-sm';
        ...
        const nameLabel = adj.name ? `<div class="font-bold text-slate-800 mb-0.5">${adj.name}</div>` : '';
        ...
        adjDiv.innerHTML = `
            ...
            <!-- Bottom Section: Timestamp & Buttons (Right Aligned) -->
            <div class="flex justify-end items-center gap-3 mt-2">

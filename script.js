/**
 * BillBuddy - Shared Expense Manager
 * 
 * @version 1.0.10
...
let collapsedSummaryDays = new Set(); // Tracks which Day labels are currently collapsed
...
const loadingText = document.getElementById('loading-text'); // New: Loading text element
...
onAuthStateChanged(auth, async user => {
    ...
    if (user) {
        // User is logged in
        showLoading("Logging you in..."); // Show loading spinner on login
        ...
        hideLoading(); // Hide loading spinner after dashboard/trip is loaded
    } else {
        ...
        hideLoading(); // Hide loading spinner if authentication fails or user logs out
...
async function handleInvitation(tripId, user) {
    try {
        showLoading("Joining trip..."); // Show loading for invitation
...
loginForm.addEventListener('submit', e => {
    ...
    showLoading("Logging in..."); // Show loading on explicit login attempt
    // signInWithEmailAndPassword now takes the auth instance as its first argument
    signInWithEmailAndPassword(auth, email, password)
        .catch(error => {
            showError(`Login Failed: ${error.message}`);
            hideLoading(); // Hide loading if login fails
        });
});
...
registerForm.addEventListener('submit', e => {
    ...
    showLoading("Registering account..."); // Show loading on registration
    // createUserWithEmailAndPassword now takes the auth instance as its first argument
    createUserWithEmailAndPassword(auth, email, password)
        .catch(error => {
            showError(`Registration Failed: ${error.message}`);
            hideLoading(); // Hide loading if registration fails
        });
});
...
async function handlePasswordReset(e) {
    ...
    if (email) {
        showLoading("Sending reset email..."); // Show loading for password reset
        ...
        sendPasswordResetEmail(auth, email)
            ...
            .finally(() => {
                hideLoading(); // Hide loading after password reset attempt
            });
    }
}
...
logoutLink.addEventListener('click', (e) => {
    ...
    showLoading("Logging out..."); // Show loading on logout
...
async function loadUserDashboard(userId) {
    showLoading("Loading your dashboard..."); // Show loading when loading dashboard
    ...
    hideLoading(); // Hide loading after dashboard is loaded
}

async function loadTrip(tripId) {
    showLoading("Loading trip data..."); // Show loading when loading a specific trip
    ...
    hideLoading(); // Hide loading after trip is loaded
}
    ...
    if (currentTripRef) {
        ...
        dataUnsubscribe = onValue(currentTripRef, async (snapshot) => { // Modular onValue
            ...
            appData = {
               ...
               customRates: data.customRates || {} // <--- ADD THIS
            };
...
async function createNewTrip() {
    ...
    showLoading("Creating new trip..."); // Show loading for new trip creation
    ...
    try {
        ...
        hideLoading(); // Hide loading after new trip is loaded

    } catch (error) {
        showError(`Failed to create trip: ${error.message}`);
        hideLoading(); // Hide loading if trip creation fails
    }
}
...
async function updateAndPopulateTripSwitcher() {
    ...
    trips.forEach(trip => {
        ...
        item.innerHTML = `
            ...
            <button class="copy-trip-link-btn text-slate-500 hover:text-sky-500 transition-colors shrink-0 mr-3" 
                    data-trip-id="${trip.id}" 
                    data-trip-name="${trip.name}" 
                    aria-label="Share link for ${trip.name}">
                <i class="fa-brands fa-whatsapp text-lg"></i>
            </button>

            <!-- 2. Trip Name (Middle - Grows to fill space) -->
            <span class="truncate grow text-left font-medium text-slate-700">${trip.name}${ownedByLabel}</span>
            
            <!-- 3. Down Arrow (Right) -->
            <button class="toggle-members-btn text-slate-500 hover:text-sky-500 transition-colors shrink-0 ml-2 p-1" 
                    aria-label="Show members for ${trip.name}">
                <i class="fas fa-chevron-down text-base transition-transform duration-200"></i>
            </button>
        `;
...
function attachTripSwitcherListeners() {
    document.querySelectorAll('.copy-trip-link-btn').forEach(btn => {
        btn.addEventListener('click', (event) => {
            ...
            const tripName = event.currentTarget.dataset.tripName; // Get trip name
            if (tripIdToCopy && tripName) {
                const shareLink = `${window.location.origin}${window.location.pathname}?shared=${tripIdToCopy}`;
                const messageToPrepend = `Click to join *${tripName}* now! 🗺️😊\n`;
                const textToCopy = messageToPrepend + shareLink;
                copyToClipboard(textToCopy, "Trip invitation link copied! Ready to paste. 📋");
            }
        });
    });
    ...
    document.querySelectorAll('.kick-member-btn').forEach(btn => {
        btn.addEventListener('click', (event) => {
            ...
            showConfirmation(`Are you sure you want to remove ${memberEmail} from this trip?`, () => {
                showLoading("Removing member..."); // Show loading for member removal
...
async function removeMemberFromTrip(tripId, memberIdToRemove, memberEmail) {
    try {
        ...
        hideLoading(); // Hide loading after member removal attempt
    }
}
...
async function deleteTrip(tripId, tripName, memberIds) {
    showConfirmation(`Are you sure you want to permanently delete '${tripName}'? This action will remove the trip for ALL members and cannot be undone.`, () => {
        showLoading("Deleting trip..."); // Show loading for trip deletion
        try {
            const updates = {};
            ...
            update(ref(database, '/'), updates).then(async () => {
                ...
                hideLoading(); // Hide loading after trip deletion attempt
            });
        } catch (error) {
            showError(`Failed to delete trip: ${error.message}`);
            console.error("Error deleting trip:", error);
            hideLoading(); // Hide loading if an immediate error occurs
        }
    });
}
...
function saveDataToFirebase() {
    ...
    const dataToUpdate = {
        ...
        customRates: appData.customRates || {} // <--- ADD THIS
    };
...
function updateAllUI() {
    ...
    // 1. Update Basic Form Inputs
    groupEventNameInput.value = appData.groupEventName;
    tripPrimaryCurrencySelect.value = appData.primaryCurrency;

    // 2. Update Header Trip Switcher Button
    const tripNameDisplay = appData.groupEventName || 'Untitled Trip';

    // --- ALIGNMENT & SPACING FIX ---
    // 'ml-auto': Pushes button to the far right
    // 'pl-3 pr-3': Adds spacing on the left and right inside the button
    // 'py-1.5': Adds vertical spacing for a better button shape
    tripSwitcherBtn.classList.add('ml-auto', 'pl-3', 'pr-3', 'py-1.5');
    
    // Ensure the button acts as a flex container
    tripSwitcherBtn.classList.add('flex', 'items-center', 'gap-2');

    // Reconstruct the button HTML: [WhatsApp] [Name] [Arrow]
    tripSwitcherBtn.innerHTML = `
        <!-- WhatsApp Share Trigger -->
        <span id="header-share-trigger" 
              class="flex items-center justify-center w-7 h-7 rounded-full bg-white/20 hover:bg-white/40 text-white transition-colors z-20 shadow-sm ring-1 ring-white/10 cursor-pointer shrink-0" 
              title="Copy Invitation Link">
            <i class="fa-brands fa-whatsapp text-sm"></i>
        </span>
        
        <!-- Trip Name -->
        <!-- UPDATED: Increased max-w-[120px] to max-w-[180px] for mobile -->
        <span class="truncate max-w-[180px] sm:max-w-[300px] font-medium text-white text-shadow-sm text-center">
            ${tripNameDisplay}
        </span>
        
        <!-- Dropdown Arrow -->
        <i class="fas fa-chevron-down text-xs text-white/80 shrink-0 ml-1"></i>
    `;

    // Attach Event Listener for the new Header WhatsApp Button
    const headerShareBtn = document.getElementById('header-share-trigger');
    if (headerShareBtn) {
        headerShareBtn.addEventListener('click', (e) => {
            // STOP the click from bubbling up to tripSwitcherBtn (which opens the modal)
            e.stopPropagation(); 
            
            if (currentTripId) {
                const shareLink = `${window.location.origin}${window.location.pathname}?shared=${currentTripId}`;
                const messageToPrepend = `Click to join *${tripNameDisplay}* now! 🗺️😊\n`;
                copyToClipboard(messageToPrepend + shareLink, "Trip invitation link copied! 📋");
            }
        });
    }

    // 3. Trigger Sub-Component Updates
    ...
    // 4. Show/hide reset button based on ownership
...
// Fetches real-time exchange rates or uses fallbacks + custom overrides
async function fetchExchangeRates() {
    const baseCurrency = appData.primaryCurrency;
    
    // 1. Fetch Standard Rates (API or Fallback)
    try {
        ...
        console.warn('API Error, using fallbacks:', error);
        exchangeRates = {};
    ...
    // App uses format: 1 Primary = X Foreign (e.g. 1 HKD = 20 JPY)
    // Custom Rates in appData are stored in this format.
    if (appData.customRates) {
        for (const [currency, rate] of Object.entries(appData.customRates)) {
            exchangeRates[currency] = rate;
            console.log(`Using custom rate for ${currency}: ${rate}`);
        }
    }
...
// ==========================================
// 1. Custom Rate Variables & Configuration
// ==========================================
const customRateModal = document.getElementById('custom-rate-modal');
const customRateSource = document.getElementById('custom-rate-source');
const customRateTarget = document.getElementById('custom-rate-target');
const customRateInput = document.getElementById('custom-rate-input');
const customRatePreview = document.getElementById('custom-rate-preview');
const supportedCurrencies = ['HKD','CNY','GBP','JPY','THB','TWD','KRW','SGD','USD','EUR','AUD','CAD','NZD','CHF','MYR','VND','PHP','IDR'];

// ==========================================
// 2. Main Currency Dropdown Listener
// ==========================================
tripPrimaryCurrencySelect.addEventListener('change', async () => {
    const selectedValue = tripPrimaryCurrencySelect.value;
    const currentPrimary = appData.primaryCurrency;

    // ===============================================
    // PATH A: User selected "Set Custom Rate"
    // ===============================================
    if (selectedValue === 'CUSTOM') {
        // 1. Immediately reset the dropdown visual
        tripPrimaryCurrencySelect.value = currentPrimary;

        // 2. Populate Source Dropdown
        customRateSource.innerHTML = '';
        
        let firstSavedCurrency = null; // To track the first currency with a saved rate

        // --- Flag Mapping for consistent UI ---
        const currencyFlags = {
            HKD: '🇭🇰', CNY: '🇨🇳', GBP: '🇬🇧', JPY: '🇯🇵', THB: '🇹🇭', TWD: '🇹🇼',
            KRW: '🇰🇷', SGD: '🇸🇬', USD: '🇺🇸', EUR: '🇪🇺', AUD: '🇦🇺', CAD: '🇨🇦',
            NZD: '🇳🇿', CHF: '🇨🇭', MYR: '🇲🇾', VND: '🇻🇳', PHP: '🇵🇭', IDR: '🇮🇩'
        };

        // ... inside PATH A: User selected "Set Custom Rate" ...
        supportedCurrencies.forEach(code => {
            if (code !== currentPrimary) {
                const option = document.createElement('option');
                option.value = code;
                
                // Get flag emoji from mapping
                const flag = currencyFlags[code] || '';
                let displayText = `${flag} ${code}`;
                
                // CHECK: Is there a saved custom rate?
                if (appData.customRates && appData.customRates[code]) {
                    option.textContent = `${displayText} *`; 
                    option.classList.add('font-bold', 'text-sky-700');
                    if (!firstSavedCurrency) firstSavedCurrency = code;
                } else {
                    option.textContent = displayText;
                }
                
                customRateSource.appendChild(option);
            }
        });

        // 3. Auto-Select Logic
        if (firstSavedCurrency) {
            customRateSource.value = firstSavedCurrency;
        }

        // 4. Set Target Input (Now with Flag)
        // Pull the flag emoji from our currencyFlags mapping
        const targetFlag = currencyFlags[currentPrimary] || '';
        customRateTarget.value = `${targetFlag} ${currentPrimary}`;

        // 5. Populate Input Field (Based on the now-selected source)
        const currentSource = customRateSource.value;
        if (appData.customRates && appData.customRates[currentSource]) {
            const savedUserRate = 1 / appData.customRates[currentSource];
            customRateInput.value = parseFloat(savedUserRate.toFixed(6));
            setTimeout(updateCustomPreview, 0); 
        } else {
            customRateInput.value = '';
            customRatePreview.textContent = 'Enter a rate above...';
        }

        // 6. Show the Modal
        customRateModal.classList.remove('hidden');
        requestAnimationFrame(() => {
            customRateModal.classList.remove('opacity-0');
            customRateModal.querySelector('div').classList.remove('scale-95');
        });
        return; 
    }

    // ===============================================
    // PATH B: User selected a Standard Currency
    // ===============================================
    if (selectedValue === currentPrimary) return;

    tripPrimaryCurrencySelect.value = currentPrimary;

    try {
        showLoading("Checking exchange rates...");
        let newRates = {};
        try {
            const response = await fetch(`https://v6.exchangerate-api.com/v6/${API_KEY}/latest/${selectedValue}`);
            const data = await response.json();
            if (data.result === 'success' && data.conversion_rates) {
                newRates = data.conversion_rates;
            } else {
                throw new Error("API Error");
            }
        } catch (e) {
            const primaryInUsd = fallbackRates[selectedValue];
            for (const key in fallbackRates) {
                newRates[key] = fallbackRates[key] / primaryInUsd;
            }
        }

        hideLoading();
        const rateOldInNewBase = newRates[currentPrimary] || 1; 
        const liveRate = 1 / rateOldInNewBase; 

        const message = `
            Switching from <strong>${currentPrimary}</strong> to <strong>${selectedValue}</strong>.
            <div class="bg-slate-50 p-3 rounded-md mt-3 mb-3 text-sm border border-slate-200 text-center">
                <span class="text-slate-500 mr-2">Today's Rate:</span>
                <span class="font-mono font-bold text-slate-700">1 ${currentPrimary} = ${liveRate.toFixed(4)} ${selectedValue}</span>
            </div>
            <p class="text-xs text-slate-500">⚠️ All existing expenses will be recalculated using this rate.</p>
        `;
        ...
        showConfirmation(
            ...
            async () => {
                try {
                    showLoading("Converting expenses...");
                    appData.primaryCurrency = selectedValue;
                    exchangeRates = newRates; 
                    appData.customRates = {}; // Clear custom rates on primary switch

                    appData.expenses = appData.expenses.map(item => {
                        if (item.type === 'expense') {
                            const rateOfOriginalInNewBase = newRates[item.currency] || 1;
                            const newAmountInPrimary = item.amount / rateOfOriginalInNewBase;
                            const newPerBuddy = newAmountInPrimary / item.buddies.length;
                            return { ...item, amountInPrimaryCurrency: newAmountInPrimary, perBuddy: newPerBuddy };
                        }
                        return item;
                    });

                    saveDataToFirebase();
                    tripPrimaryCurrencySelect.value = selectedValue;
                    showError(`Converted to ${selectedValue}. Expenses updated.`, 3000, 'success');
                } catch (error) {
                    console.error("Conversion error:", error);
                    showError("Error converting expenses.");
                    appData.primaryCurrency = currentPrimary;
                    tripPrimaryCurrencySelect.value = currentPrimary;
                    await fetchExchangeRates(); 
                } finally {
                    hideLoading();
        ...
        console.error(error);
        hideLoading();
        showError("Could not fetch exchange rates. Try again.");
    }
});

// ==========================================
// 3. Custom Rate Modal Logic
// ==========================================

// A. Real-time Preview Calculation
function updateCustomPreview() {
    const source = customRateSource.value; // This is the 3-letter code (e.g., GBP)
    const target = appData.primaryCurrency; // Use raw code from data (e.g., HKD) instead of input value
    const rate = parseFloat(customRateInput.value);

    if (!rate || isNaN(rate)) {
        customRatePreview.textContent = 'Enter a rate to see calculation';
        return;
    }

    // Example Calculation: 100 Source * Rate = X Target
    const exampleAmount = 100;
    const result = exampleAmount * rate;
    
    customRatePreview.innerHTML = `
        <span class="font-bold">${exampleAmount} ${source}</span> = 
        <span class="font-bold text-sky-600">${result.toLocaleString(undefined, {maximumFractionDigits:2})} ${target}</span>
    `;
}

// B. Listeners
if (customRateInput) customRateInput.addEventListener('input', updateCustomPreview);

// NEW: When changing currency, check for saved rates
if (customRateSource) {
    customRateSource.addEventListener('change', () => {
        const source = customRateSource.value;
        // Check if we have a saved rate for this currency
        if (appData.customRates && appData.customRates[source]) {
            const savedUserRate = 1 / appData.customRates[source];
            customRateInput.value = parseFloat(savedUserRate.toFixed(6));
        } else {
            // No saved rate, clear the input
            customRateInput.value = '';
        }
        updateCustomPreview();
    });
...
// C. Close Modal
document.getElementById('close-custom-rate-btn').addEventListener('click', () => {
    customRateModal.classList.add('opacity-0');
    customRateModal.querySelector('div').classList.add('scale-95');
    setTimeout(() => customRateModal.classList.add('hidden'), 300);
});

// D. Save Custom Rate
document.getElementById('save-custom-rate-btn').addEventListener('click', () => {
    const sourceCurrency = customRateSource.value; 
    const inputRate = parseFloat(customRateInput.value); 

    if (!inputRate || inputRate <= 0) {
        showError("Please enter a valid rate.");
        return;
    }
    const appSystemRate = 1 / inputRate;

    if (!appData.customRates) appData.customRates = {};
    appData.customRates[sourceCurrency] = appSystemRate;

    showLoading("Recalculating expenses...");

    if (!exchangeRates) exchangeRates = {};
    exchangeRates[sourceCurrency] = appSystemRate;

    appData.expenses = appData.expenses.map(item => {
        if (item.type === 'expense' && item.currency === sourceCurrency) {
            const newAmountInPrimary = item.amount / appSystemRate;
            const newPerBuddy = newAmountInPrimary / item.buddies.length;
            return { ...item, amountInPrimaryCurrency: newAmountInPrimary, perBuddy: newPerBuddy };
        }
        return item;
    ...
    setTimeout(() => {
        ...
        hideLoading();
        showError(`Custom rate set for ${sourceCurrency}!`, 3000, 'success');
        updateAllUI(); 
    }, 300);
});

// E. Fetch Live Rate Button Logic
document.getElementById('fetch-live-rate-btn').addEventListener('click', async () => {
    const source = customRateSource.value; 
    const target = customRateTarget.value; 
    const btn = document.getElementById('fetch-live-rate-btn');
    const icon = btn.querySelector('i');
    ...
    try {
        ...
        if (data.result === 'success' && data.conversion_rates) {
            ...
            if (rate) {
                customRateInput.value = parseFloat(rate.toFixed(6));
                updateCustomPreview();
                showError("Fetched current market rate.", 2000, 'success');
            } else {
        ...
        try {
            const calculatedFallback = fallbackRates[target] / fallbackRates[source];
            customRateInput.value = parseFloat(calculatedFallback.toFixed(6));
            updateCustomPreview();
            showError("Network error. Used estimated rate.", 3000);
        } catch (e) {
            showError("Could not fetch rate.");
        }
    } finally {
...
doneBuddiesBtn.addEventListener('click', () => {
    ...
    if (!eventName) { showError('Please set a trip name on the Buddies tab first.'); return; }
...
function updateSummary() {
    ...
    // Removes the default 'divide-y' lines
    summaryTableBody.className = ''; 
    ...
    // Step 1: Pre-calculate daily subtotals per buddy
    ...
    // Step 2: Build the details HTML per buddy
    appData.buddies.forEach(buddy => {
        let activeDayLabel = null;

        appData.expenses.forEach(item => {
            if (item.type === 'divider') {
                if (activeDayLabel !== null) details[buddy].push('</div>');

                activeDayLabel = item.label;
                const subtotal = buddyDailySubtotals[buddy]?.[activeDayLabel] || 0;
                const subtotalStr = subtotal > 0 ? ` - Subtotal ${currencySymbol}${formatCurrency(subtotal)}` : '';
                
                const isCollapsed = collapsedSummaryDays.has(activeDayLabel);
                const icon = isCollapsed ? 'fa-chevron-right' : 'fa-chevron-down';
                const hiddenClass = isCollapsed ? 'hidden' : '';

                // Apply consistent spacing for all days
                const borderClasses = 'mt-1 mb-1'; 

                details[buddy].push(`
                    <div class="text-xs font-bold text-sky-700 cursor-pointer flex items-center day-toggle-btn ${borderClasses}" data-day="${activeDayLabel}">
                        <i class="fas ${icon} mr-2 text-[10px]"></i>
                        <span>${activeDayLabel}${subtotalStr}</span>
                    </div>
                    <div class="day-content-area ${hiddenClass}">
                `);

            } else if (item.type === 'expense') {
                const exp = item;
                if (exp.buddies.includes(buddy)) {
                    totals[buddy] += exp.perBuddy;
                    details[buddy].push(`
                        <div class="text-xs text-slate-600">
                            <span class="font-medium">${exp.mealType}</span> at ${exp.restaurant}: <span class="font-bold text-slate-700">${currencySymbol}${formatCurrency(exp.perBuddy)}</span>
                        </div>
                    `);
                }
            }
    ...
    if (appData.buddies.length === 0) {
        summaryTableBody.innerHTML = `<tr><td colspan="2" class="py-4 px-2 text-center text-slate-500">No buddies added yet.</td></tr>`;
        return;
    }

    // Step 3: Populate Table Rows (Two-Row Layout with separators)
    appData.buddies.forEach((buddy, index) => {
        // Row 1: Buddy Name (Left) and Total (Right)
        const rowHeader = summaryTableBody.insertRow();
        rowHeader.className = index > 0 ? "border-t border-slate-200" : "border-none";
        
        rowHeader.innerHTML = `
            <!-- Left: Buddy Name -->
            <td class="pt-4 px-2 md:px-4 text-slate-800 font-bold text-sm align-bottom text-left">${buddy}</td>
            
            <!-- Right: Total Owed (Updated to match Buddy Name size/weight) -->
            <td class="pt-4 px-2 md:px-4 text-slate-700 font-bold text-sm align-bottom text-right">${currencySymbol}${formatCurrency(totals[buddy])}</td>
        `;

        // Row 2: Details (Full Width)
        const rowDetails = summaryTableBody.insertRow();
        rowDetails.className = "border-none";
        rowDetails.innerHTML = `
            <td colspan="2" class="pb-4 px-2 md:px-4 w-full">
                ${details[buddy].join('') || '<div class="text-slate-400 text-xs mt-1">No records</div>'}
            </td>
        `;
    });
}
    ...
    appData.expenses.forEach((item) => {
        ...
        if (item.type === 'divider') {
            ...
            div.innerHTML = `
                ...
                <button class="delete-divider absolute top-2 right-2 bg-rose-100 text-rose-600 rounded-full w-6 h-6 flex items-center justify-center text-xs font-bold hover:bg-rose-200 transition-colors" data-id="${item.id}" aria-label="Delete divider">✖</button>
            `;
        } else if (item.type === 'expense') {
            const exp = item;
            div = document.createElement('div');
            div.className = 'meal-record-item record-list-item'; 
            div.draggable = true;
            div.dataset.id = exp.id;
            const timestamp = formatTimestamp(exp.timestamp);
            
            // New Layout:
            // Top: Title (Left) + Price (Right)
            // Middle: Paid By + Shared With
            // Bottom: Timestamp + Action Buttons (Right Aligned)
            div.innerHTML = `
                <div class="flex justify-between items-start gap-3 mb-1">
                    <div class="font-semibold text-slate-800 text-sm break-words min-w-0">
                        ${exp.mealType} @ ${exp.restaurant}
                    </div>
                    <div class="text-sky-600 font-bold text-sm whitespace-nowrap shrink-0">
                        ${exp.currency} ${formatCurrency(exp.amount)}
                    </div>
                </div>
                
                <div class="text-xs mt-1">Paid by: <strong class="text-slate-700">${exp.payer}</strong></div>
                ...
                <div class="flex justify-end items-center gap-3 mt-2">
                    <div class="text-slate-400 text-[10px]">${timestamp}</div>
                    <div class="flex gap-1.5">
                        <button class="edit-meal-record bg-sky-100 text-sky-600 rounded-full w-6 h-6 flex items-center justify-center text-xs font-bold hover:bg-sky-200 transition-colors" data-id="${exp.id}" aria-label="Edit meal record">✏️</button>
                        <button class="delete-meal-record bg-rose-100 text-rose-600 rounded-full w-6 h-6 flex items-center justify-center text-xs font-bold hover:bg-rose-200 transition-colors" data-id="${exp.id}" aria-label="Delete meal record">✖</button>
                    </div>
                </div>`;
        }
...
function editAdjustment(index) {
    const adj = appData.adjustments[index];
    if (!adj) return;

    editingAdjustmentIndex = index;

    // Populate form fields
    document.getElementById('adjustment-name').value = adj.name || '';
    document.getElementById('adjustment-debtor').value = adj.debtor;
    filterCreditorDropdown(); // Refresh dropdown to ensure creditor is available
    document.getElementById('adjustment-creditor').value = adj.creditor;
    document.getElementById('adjustment-currency').value = adj.originalCurrency;
    document.getElementById('adjustment-amount').value = adj.amount;
    
    // Reset calculator display
    const calcInput = document.getElementById('calculator-input');
    if (calcInput) calcInput.innerHTML = '';
    isCalculatorResultShown = false;

    // Change button UI to update mode
    const addAdjBtn = document.getElementById('add-adjustment-btn');
    addAdjBtn.innerHTML = '<span class="text-lg">💾</span> Update Adjustment';
    addAdjBtn.classList.remove('bg-indigo-500', 'hover:bg-indigo-600');
    addAdjBtn.classList.add('bg-amber-500', 'hover:bg-amber-600');

    // Scroll to the adjustment form for better UX
    document.getElementById('adjustment-debtor').scrollIntoView({ behavior: 'smooth', block: 'center' });
}
...
function updateAdjustmentsDisplay() {
    ...
    appData.adjustments.forEach((adj, index) => {
        ...
        adjDiv.className = 'bg-white p-3.5 rounded-lg shadow-sm border border-slate-200 text-sm';
        ...
        const timestampStr = adj.timestamp ? formatTimestamp(adj.timestamp) : '';
        const nameText = adj.name || ''; // Allow empty name to maintain layout structure

        adjDiv.innerHTML = `
            <!-- Top Row: Title (Bottom-Left) + Buttons (Top-Right) -->
            <div class="flex justify-between mb-1">
                <div class="font-bold text-slate-800 break-words min-w-0 mr-2 self-end">${nameText}</div>
                <div class="flex gap-1.5 shrink-0 self-start">
                    <button class="edit-adjustment-btn bg-sky-100 text-sky-600 rounded-full w-6 h-6 flex items-center justify-center text-xs font-bold hover:bg-sky-200 transition-colors" data-index="${index}" aria-label="Edit adjustment">✏️</button>
                    <button class="delete-adjustment-btn bg-rose-100 text-rose-600 rounded-full w-6 h-6 flex items-center justify-center text-xs font-bold hover:bg-rose-200 transition-colors" data-index="${index}" aria-label="Delete adjustment">✖</button>
                </div>
            </div>
            ...
            ${timestampStr ? `<div class="text-slate-400 text-[10px]">${timestampStr}</div>` : ''}
        `;
        container.appendChild(adjDiv);
    });

    // Attach Listeners
    document.querySelectorAll('.edit-adjustment-btn').forEach(btn => {
        btn.addEventListener('click', (event) => {
            const indexToEdit = parseInt(event.currentTarget.dataset.index);
            editAdjustment(indexToEdit);
        });
    });
    ...
    document.querySelectorAll('.delete-adjustment-btn').forEach(btn => {
        btn.addEventListener('click', (event) => {
            ...
            if (adjToDelete) {
                const confirmationMessage = `Are you sure you want to remove the '${adjToDelete.name || 'adjustment'}'?`;
                showConfirmation(confirmationMessage, () => {
...
document.getElementById('add-adjustment-btn').addEventListener('click', () => {
    const name = document.getElementById('adjustment-name').value.trim();
    const debtor = document.getElementById('adjustment-debtor').value;
    const creditor = document.getElementById('adjustment-creditor').value;
    ...
    if (!debtor || !creditor || !currency || isNaN(amount) || amount <= 0) {
        showError('Please select payer, receiver, currency, and a valid amount.'); 
        return;
    }
    if (debtor === creditor) {
        showError('Payer and Receiver cannot be the same buddy.'); 
        return;
    }

    // Determine timestamp: Use existing if editing, otherwise create new
    let timestamp;
    if (editingAdjustmentIndex > -1) {
        // Keep the original timestamp to preserve history
        timestamp = appData.adjustments[editingAdjustmentIndex].timestamp || new Date().toISOString();
    } else {
        // New entry gets current time
    ...
    if (editingAdjustmentIndex > -1) {
        // Update existing
        appData.adjustments[editingAdjustmentIndex] = adjustmentData;
        showError('Adjustment updated successfully! ✨', 3000, 'success');
    } else {
        // Add new
        appData.adjustments.push(adjustmentData);
        showError('Adjustment added successfully! ➕', 3000, 'success');
    }

    // Reset State and UI
    editingAdjustmentIndex = -1;
    addAdjBtn.innerHTML = '<span class="text-lg">➕</span>Add Adjustment';
    addAdjBtn.classList.remove('bg-amber-500', 'hover:bg-amber-600');
    addAdjBtn.classList.add('bg-indigo-500', 'hover:bg-indigo-600');

    // Clear form
    document.getElementById('adjustment-name').value = '';
    document.getElementById('adjustment-amount').value = '';
    document.getElementById('adjustment-debtor').value = '';
    document.getElementById('adjustment-creditor').value = '';
    const calcInput = document.getElementById('calculator-input');
    if (calcInput) calcInput.innerHTML = '';
    isCalculatorResultShown = false;
...
function initGlobalFooter() {
    const footerHTML = `
        <div class="w-full mt-4 pb-4 text-center text-xs text-slate-400 font-medium tracking-wide">
            Designed & Powered by 
            <a href="https://28carparts.com" target="_blank" class="text-slate-400 hover:text-sky-500 transition-all duration-200 font-bold">
                28CarParts
            </a>
        </div>
    `;

    // 1. Auth Page Alignment
    const authWrapper = document.querySelector('#page-auth .w-full.max-w-md');
    if (authWrapper && !authWrapper.querySelector('.global-footer')) {
        const footer = document.createElement('div');
        footer.className = 'global-footer';
        footer.innerHTML = footerHTML;
        authWrapper.appendChild(footer);
    }

    // 2. App Pages Alignment (Buddies, Expense, Summary, Debt)
    // We inject the footer into each individual page section so it respects the same padding
    const appPages = document.querySelectorAll('.page');
    appPages.forEach(page => {
        // Find the inner white card wrapper
        const cardWrapper = page.querySelector('.max-w-2xl.mx-auto') || page.querySelector('.max-w-4xl.mx-auto');
        if (cardWrapper && !page.querySelector('.global-footer')) {
            const footer = document.createElement('div');
            footer.className = 'global-footer';
            footer.innerHTML = footerHTML;
            // Place it right after the white card, but inside the section padding
            cardWrapper.after(footer);
        }
    });

    // 3. No Trips Page Alignment
    const noTripsWrapper = document.querySelector('#page-no-trips .max-w-md');
    if (noTripsWrapper && !noTripsWrapper.querySelector('.global-footer')) {
        const footer = document.createElement('div');
        footer.className = 'global-footer';
        footer.innerHTML = footerHTML;
        noTripsWrapper.appendChild(footer);
    }
}
...
// Attach event delegation for the summary table toggles
document.getElementById('summary-table').addEventListener('click', (e) => {
    const btn = e.target.closest('.day-toggle-btn');
    if (btn) {
        const dayLabel = btn.dataset.day;
        if (collapsedSummaryDays.has(dayLabel)) {
            collapsedSummaryDays.delete(dayLabel);
        } else {
            collapsedSummaryDays.add(dayLabel);
        }
        updateSummary(); // Re-render to sync all buddies
    }
});